...
function Courses(props) {
    ...
    useEffect(async () => {
        ...
        const {data: Semesters} = await api.courses.getDisciplineStudentSemester(ListGroups[0].idGroup)
        setSemesters(Semesters)
        setChosenSemester(Semesters[0].idSemestr)

        const {data: ListDisciplines} = await api.courses.getStudyLoadStudent(ListGroups[0].idGroup, Semesters[0].idSemestr)
    ...
    return (
        <Grid container spacing={3}>
            ...
            <Grid item xs={12}>
                <Typography className={classesMain.Title}>Курсы по учебному плану</Typography>
                <CustomAlert
                    severity="info"
                    title='Информация!'
            ...
            >
                {
                    listGroups.map(item =>
                        <MenuItem className={classesMain.SelectItems}
                                  value={item.idGroup} key={item.idGroup}>{item.groupName}</MenuItem>
                    )
                }
            </CustomSelect>
